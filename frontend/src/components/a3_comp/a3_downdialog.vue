...
export default {
  ...
  methods: {
    ...
    async downmessage() {
      ...
      if (val == 2) {
        const temp1 = JSON.parse(localStorage.getItem("from_branch"));
        const option = this.option;
        const Chinese = this.Chinese;
        this.filename = "支部学生信息下载.xlsx";
        ...
        const temp2 = JSON.parse(localStorage.getItem("from_college"));
        const option = this.option;
        const Chinese = this.Chinese;
        this.filename = "学院学生信息下载.xlsx";
        const res = await this.$http.post(
          "export/college/" + temp2,
          { option, Chinese },
          {
            params: this.filter,
            responseType: "blob",
