    ...
    <el-card class="box-card">
      ...
      >
        ...
        >
          ...
          <br />
          <br />
          <span style="font-weight: bold">发展对象模板</span>
          <br />
          <br />
          <el-image :src="require('../../assets/demo/demo_4.png')"></el-image>
          <br />
          <span style="font-weight: bold">发展对象评优情况模板</span>
          <el-image :src="require('../../assets/demo/demo_5.png')"></el-image>
          <br />
          <br />
          ...
          <span slot="footer" class="dialog-footer">
            ...
            >
              <el-button type="primary" @click="temp = 0">发展对象</el-button>
              <el-button type="primary" @click="temp = 1"
                >发展对象培训班评优情况</el-button
              >
            </el-upload>
          </span>
        </el-dialog>
  ...
  methods: {
    ...
    async modeUpload(item) {
      ...
      if (this.temp == 0) {
        ...
        res = await this.$http.post("impo/developer/" + this.branch_id, fd);
      } else {
        let fd = new FormData();
        fd.append("file_obj", this.mode);
        res = await this.$http.post(
          "impo/developer_appraising/" + this.branch_id,
          fd
        );
      }
    ...
    async getUserList() {
      ...
      if (val == 2) {
        ...
        this.temp = await this.$http.get("user/branch/3/" + this.branch_id);
        this.copy_show = true;
      } else if (val == 3) {
        this.upload_show = false;
        const val2 = JSON.parse(localStorage.getItem("from_college"));
        this.temp = await this.$http.get("user/college/3/" + val2);
      } else if (val == 4) {
        this.upload_show = false;
        this.temp = await this.$http.get("user/list/3");
      }
    ...
    showchild(account) {
      this.$refs.a3_change.showEditDialog(account);
      this.getUserList();
    },
    // 根据Id删除用户
