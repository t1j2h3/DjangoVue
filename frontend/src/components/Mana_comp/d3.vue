  ...
  methods: {
    ...
    async getUserList() {
      ...
      if (val == 2) {
        ...
        this.temp = await this.$http.get("user/branch/4/" + this.branch_id);
        this.copy_show = true;
      } else if (val == 3) {
        this.upload_show = false;
        const val2 = JSON.parse(localStorage.getItem("from_college"));
        this.temp = await this.$http.get("user/college/4/" + val2);
      } else if (val == 4) {
        this.upload_show = false;
        this.temp = await this.$http.get("user/list/4");
      }
    ...
    uploadfile() {
      ...
      this.$http
        .post("impo/probation/" + this.branch_id, fd)
        .then((response) => {
