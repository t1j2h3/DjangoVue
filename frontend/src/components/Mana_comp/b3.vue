    ...
    <el-card class="box-card">
      ...
      >
        ...
        >
          ...
          <br />
          <br />
          <el-image :src="require('../../assets/demo/demo_2.png')"></el-image>
          <br />
          <span style="font-weight: bold">入党申请人模板</span>
          <el-image :src="require('../../assets/demo/demo_3.png')"></el-image>
          <br />
          <br />
          ...
          <span slot="footer" class="dialog-footer">
            ...
            >
              <el-button type="primary" @click="temp = 0">积极分子</el-button>
  ...
  methods: {
    ...
    async modeUpload(item) {
      ...
      if (this.temp == 0) {
        ...
        res = await this.$http.post("impo/activists/" + this.branch_id, fd);
      } else {
        let fd = new FormData();
        fd.append("file_obj", this.mode);
        res = await this.$http.post(
          "impo/activists_appraising/" + this.branch_id,
          fd
        );
      }
    ...
    async getUserList() {
      ...
      if (val == 2) {
        ...
        this.temp = await this.$http.get("user/branch/2/" + this.branch_id);
        this.copy_show = true;
      } else if (val == 3) {
        this.upload_show = false;
        const val2 = JSON.parse(localStorage.getItem("from_college"));
        this.temp = await this.$http.get("user/college/2/" + val2);
      } else if (val == 4) {
        this.upload_show = false;
        this.temp = await this.$http.get("user/list/2");
